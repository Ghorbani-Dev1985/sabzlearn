   ...
   users.forEach((user , index) => {
    AllUsersTable.insertAdjacentHTML(
        ...
        ${index + 1}
         ...
         <td>
         <div onClick="ChangeUserRole('${user._id}')" class="text-sky-500 cursor-pointer flex-center">
         ...
         <td>
          <div onClick="BanUser('${user._id}')" class="text-zinc-900 cursor-pointer flex-center">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
     <path stroke-linecap="round" stroke-linejoin="round" d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636" />
          ...
          <td>
          <div onClick="DeleteUser('${user._id}')" class="text-rose-500 cursor-pointer flex-center">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
...
const ChangeUserRole = async (userID) => {
  ...
  Swal.fire({
    title : 'آیا برای تغییر نقش کاربری مطمعن هستید؟',
    showCancelButton: true,
    showConfirmButton: true,
    confirmButtonColor: "#3085d6",
    cancelButtonText: '#27272a',
    ...
    if(result.isConfirmed){
        Swal.fire({
          title: " نقش جدید",
          input: 'text',
          inputPlaceholder: '  لطفا نقش جدید کاربر را وارد نمایید...',
          ...
          if (result.isConfirmed && result.value !== '') {
              const UserChangeRoleInfos = {
                role: result.value,
                id : userID,
        };
               await fetch(`${BaseUrl()}users/role`, {
                method: "PUT",
                headers: {
                  Authorization: `Bearer ${GetToken()}`,
                  'Content-Type' : 'application/json',
                },
                body: JSON.stringify(UserChangeRoleInfos),
              }).then(res => {
                if(res.ok){
                  Swal.fire({
                      icon: "success",
                      title: " تغییر نقش کاربر با موفقیت ارسال شد",
                      showConfirmButton: true,
                      showCancelButton: false,
...
export {GetAndShowAllUsers , CreateNewMenu , PrepareCreateMenuFor , DeleteUser , BanUser , CreateNewUser , ChangeUserRole}