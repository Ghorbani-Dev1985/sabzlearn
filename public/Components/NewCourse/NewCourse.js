...
<div class="relative">
  <a id="NewCourseLink" class="w-full h-full block">
    ...
    <span id="NewCourseFree" class="absolute right-2.5 top-2.5 hidden w-12 h-6 bg-sabzlearnGreen text-white rounded-xl font-DanaBold text-sm">100%</span>
    ...
    <a id="NewCourseTagTwo" href="" class="inline-flex items-center justify-center text-xs text-sky-500 dark:text-yellow-400 bg-sky-500/10 dark:bg-yellow-400/10 py-1 px-1.5 mb-2.5 rounded">
      ...
      <h4 id="NewCourseTitle" class="font-DanaBold h-12 line-clamp-2 text-zinc-700 mb-3"></h4>
      <p id="NewCourseSubtitle" class="font-Dana text-sm h-10 line-clamp-2 text-slate-500 dark:text-slate-400"></p>
      <div class="flex-between gap-2.5 flex-wrap text-slate-500 dark:text-slate-400 text-xs my-3">
          ...
          <div class="w-full flex-between h-[46px]">
                ...
                <div class="flex flex-col items-center">
                 ...
                 <div class="flex-center gap-1 ">
                 <span id="NewCourseDiscountPrice" class="text-sabzlearnGreen font-DanaMd text-xl space-x-1.5"></span>
                 <img id="TomanSvg" src="./assets/images/svg/toman.svg" alt="ghorbani-dev.ir" class="w-4 h-4"/>
                 </div>
                </div>
          </div>
      </div>
...
class NewCourse extends HTMLElement {
  ...
  connectedCallback() {
    ...
    this.shadowRoot.querySelector('#NewCourseFree').classList.add(this.getAttribute('NewCourseFree'))
    this.shadowRoot.querySelector('#NewCourseTagOne').innerHTML = this.getAttribute('NewCourseTagOne')
    this.shadowRoot.querySelector('#NewCourseTagTwo').innerHTML = this.getAttribute('NewCourseTagTwo')
    this.shadowRoot.querySelector('#NewCourseTagTwo').classList.add(this.getAttribute('NewCourseTagTwoClass'))
    this.shadowRoot.querySelector('#NewCourseTitle').innerHTML = this.getAttribute('NewCourseTitle')
    this.shadowRoot.querySelector('#NewCourseSubtitle').innerHTML = this.getAttribute('NewCourseSubtitle')
    ...
    this.shadowRoot.querySelector('#NewCourseDiscountPrice').innerHTML = this.getAttribute('NewCourseDiscountPrice')
    this.shadowRoot.querySelector('#TomanSvg').classList.add(this.getAttribute('TomanSvg')) 
  }

  static observedAttributes() {
    return ['NewCourseLink' ,'NewCourseImg' , 'NewCourseFree', 'NewCourseTagOne' , 'NewCourseTagTwo' , 'NewCourseTagTwoClass ' , 'NewCourseTitle' , 'NewCourseSubtitle' , 'NewCourseTeacher' , 'NewCourseTotalTime' , 'NewCourseTotalStudents' , 'NewCoursePrice' , 'NewCourseDiscountPrice' , 'TomanSvg']
  }
}
